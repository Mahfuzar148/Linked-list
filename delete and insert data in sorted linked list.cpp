...
int main()
{
   ...
   while(1)
   {
    ...
    print(head);
    cout<<"Enter a number to insert a sorted linked list : "<<endl;
    int num1,cnt2 = 0;
    cin>>num1;
    Node* node6 = create_node(num1);
    Node* tmp8 = head;
    if((head->data)>=(node6->data))
        {
            node6->next = head;
            head = node6;
            cnt2 = 1;
       ...
       while (tmp8->next!= NULL)
        {
            if (tmp8->next->data >= node6->data)
            {
                node6->next = tmp8->next;
                tmp8->next = node6;
                cnt2 = 1;
                break;
            }
